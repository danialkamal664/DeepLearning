...
data = pd.read_csv("Datasets/delhi_weather.csv", parse_dates=[0])
print(data.head())
...
for i in range(3, len(train_data)):
    ...
    y_train.append(
        train_data[i, 0]
    )  # here 0th index is passed as it is the target variable index in our data
...
inverse_train_predictions = scaler.inverse_transform(
    np.concatenate((X_train[:, -1, 1:], train_predictions), axis=1)
)[:, -1]
inverse_test_predictions = scaler.inverse_transform(
    np.concatenate((X_test[:, -1, 1:], test_predictions), axis=1)
)[:, -1]
